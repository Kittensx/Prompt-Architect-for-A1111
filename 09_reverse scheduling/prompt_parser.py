...
!emphasized: "(" prompt ")"
        | "(" prompt ":" prompt ")"
        | "[" prompt "]"
scheduled: "[" [prompt (":" prompt)+] "]" ":" [WHITESPACE] NUMBER [WHITESPACE] ("reverse" | "r")? //extends functionality for reverse scheduler
...
numbered: NUMBER_Q ("!" "_")? "_" (grouped | sequence | compound | plain)
    ...
    def collect_steps(steps, tree):
        ...
        class CollectSteps(lark.Visitor):
            ...
            def scheduled(self, tree):
                ...
                #is_reverse = tree.children[-1].value in ("reverse", "r") if len(tree.children) > 2 else False
                # Check if the tree has enough children to include a reverse flag
                if len(tree.children) > 2:
                    # Extract the last child's value
                    reverse_flag = tree.children[-1].value
                    # Determine if the value corresponds to a reverse flag
                    is_reverse = reverse_flag.lower() in ("reverse", "r")
                else:
                    # Default to False if there are not enough children
                    is_reverse = False
                ...
                if is_reverse:
                    step_intervals.reverse()  # Reverse the scheduling intervals
                '''
    ...
    def at_step(step, tree):
        class AtStep(lark.Transformer):
            ...
            def scheduled(self, args):               
                ...
                *prompts, when, _ , is_reverse, weight = args  # Extract prompts and step boundaries
                ...
                if is_reverse:
                    prompts = prompts[::-1]  # Reverse the order of prompts #Reverses the prompts and boundaries using [::-1] slicing.
                    when = when[::-1]        # Reverse the step boundaries #Uses the reversed lists to match the current step with the appropriate boundary and prompt.
