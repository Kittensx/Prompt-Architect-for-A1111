...
!emphasized: "(" prompt ")" 
        | "(" prompt ":" prompt ")"
        | "[" prompt "]"
scheduled: "[" [prompt (":" prompt)+] "]" ":" (NUMBER | prompt) (step_range_list | reverse_flag | step_range_list reverse_flag)?
...
alternate1: prompt ("|" | "OR") prompt
alternate2: (plain | compound) ("|" (plain | compound))+
grouped: "{" ((NUMBER_Q | prompt | sequence | grouped) (","| "|" | "AND" | "OR")?)+ "}"
sequence: prompt "::" (sequence | prompt (","|WHITESPACE)* (";" | "_;")?)*  // `_;"` is now optional


compound: /[a-zA-Z0-9]+(_[a-zA-Z0-9]+)+/
numbered: NUMBER_Q ("!" "_")? "_" (grouped | sequence | compound | and_rule | plain)
and_rule: prompt ("AND") prompt+
WHITESPACE: /\s+/
plain: /([^\\\[\]():|!_&]|\\.)+/
...
def get_learned_conditioning_prompt_schedules(prompts, base_steps, hires_steps=None, use_old_scheduling=False):
    ...
    def collect_steps(steps, tree):
        ...
        def resolve_tree(tree):
            ...
            '''
            if isinstance(tree, lark.Tree):
                if tree.data == 'and_rule':
                    # Resolve all children and join with "AND"
                    resolved_children = [resolve_tree(child) for child in tree.children]
                    return " AND ".join(resolved_children)
                elif tree.data == 'sequence':
                    # Ensure sequences are flattened and resolved into strings
                    resolved_sequence = [resolve_tree(child) for child in tree.children]
                    return " ".join(resolved_sequence)
                elif tree.data == 'grouped':
                    # Ensure grouped elements are flattened and resolved into strings
                    resolved_group = [resolve_tree(child) for child in tree.children]
                    return f"{{{' '.join(resolved_group)}}}"
                elif tree.data == 'alternate1':
                    # Resolve all alternates and join them with "|"
                    resolved_alternates = [resolve_tree(child) for child in tree.children]                   
                    return random.choice(resolved_alternates)
                # For other rules, resolve recursively
                return "".join(resolve_tree(child) for child in tree.children)
        ...
        class CollectSteps(lark.Visitor):
            ...
            def grouped(self,tree):
                ...
                #res.append(", ".join(group_descriptions))
                return f"{{{', '.join(group_descriptions)}}}"
            ...
            def scheduled(self, tree):
                ...
                try:
                    v = float(resolve_tree(number_node))
                except ValueError:
                    print(f"Error: Unable to convert {number_node} to float")
                    return
